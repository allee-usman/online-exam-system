...
public class OnlineExamSystem {
    ...
    private static List<String> studentResults = new ArrayList<>();
    ...
    // Define colors
    private static final Color PRIMARY_CLR1 = new Color(0x143f40); // Dark Teal
    private static final Color PRIMARY_CLR2 = new Color(0xe0bb68); // Light Gold
    private static final Color WHITE_CLR = new Color(0xffffff); // White
    private static final Color DARK_CLR = new Color(0x111a19); // Very Dark Teal
    ...
    private static void showMainMenu() {
        ...
        mainMenuFrame.getContentPane().setBackground(WHITE_CLR); // Set background color

        JLabel titleLabel = new JLabel("Online Exam System", SwingConstants.CENTER);
        titleLabel.setFont(new Font("Arial", Font.BOLD, 18));
        titleLabel.setForeground(PRIMARY_CLR1); // Set title color
        ...
        // create panel with 4 rows and 1 column, with 10px horizontal and vertical gaps
        JPanel panel = new JPanel(new GridLayout(4, 1, 10, 10));
        panel.setBackground(WHITE_CLR); // Set panel background color
        ...
        mainMenuFrame.add(panel); // add panel to frame
        mainMenuFrame.setVisible(true); // display on screen, else created but not shown
    }
    ...
    private static void showRegistrationForm() {
        ...
        // create panel with 3 rows and 2 columns, with 10px horizontal and vertical gaps
    ...
    private static void showStudentLogin() {
        ...
        studentLoginFrame.getContentPane().setBackground(WHITE_CLR); // Set background color
        ...
        loginButton.addActionListener(e -> {
            ...
            // check if username exists, if exists indexof returns first occurence, else -1
            int index = studentUsernames.indexOf(username);
            // check if password matches at that index
    ...
    private static void showExam() {
        ...
        frame.getContentPane().setBackground(WHITE_CLR); // Set background color

        // create label with question text
        JLabel questionLabel = new JLabel("<html>" + currentQuestion.getQuestionText() + "</html>");
        questionLabel.setForeground(PRIMARY_CLR1); // Set question label color
        // create radio buttons for options
        JRadioButton[] options = new JRadioButton[4];
        // create button group to ensure only one option is selected at a time
        ButtonGroup group = new ButtonGroup();
        JPanel optionsPanel = new JPanel(new GridLayout(4, 1));
        optionsPanel.setBackground(WHITE_CLR); // Set options panel background color

        for (int i = 0; i < 4; i++) {
            options[i] = new JRadioButton(currentQuestion.getOptions()[i]);
            options[i].setBackground(WHITE_CLR); // Set radio button background color
            options[i].setForeground(DARK_CLR); // Set radio button text color
            group.add(options[i]);
    ...
    private static void showResult() {
        ...
        resultFrame.getContentPane().setBackground(WHITE_CLR); // Set background color
        ...
        resultLabel.setForeground(PRIMARY_CLR1); // Set result label color
